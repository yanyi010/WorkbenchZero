...
fn plugin_cannot_run_shell_lifecycle_methods() {
    ...
    for (method, params) in [
        ...
        (
            "plugins.uninstall",
            serde_json::json!({ "id": "zero.memo" }),
        ),
        (
            "plugins.setPinned",
            serde_json::json!({ "id": "zero.memo", "pinned": true }),
        ),
        (
        ...
        (
            "plugins.markActive",
            serde_json::json!({ "id": "zero.memo", "activationMs": 1.0 }),
        ),
        ("plugins.checkUpdates", serde_json::json!({})),
        (
            "workspace.create",
            serde_json::json!({ "name": "x", "root": "/tmp/x", "createRoot": true }),
        ),
...
fn shell_caller_is_privileged() {
    ...
    let resp = kernel.rpc(&token, req(1, "workspace.list", serde_json::json!({})));
