...
pub fn read_text(path: &Path, max: u64) -> KResult<serde_json::Value> {
    ...
    if !meta.is_file() {
        return Err(KernelError::Message(format!("`{}` is not a file", path.display())));
    }
    ...
    if truncated {
        while !text.is_empty() && text.chars().last().map(|c| c as u32 > 0x10FFFF).unwrap_or(false) {
            text.pop();
        }
    }
    Ok(serde_json::json!({
...
pub fn delete(path: &Path, recursive: bool, workspace_root: Option<&Path>) -> KResult<()> {
    if let Some(root) = workspace_root {
        if path == root {
            return Err(KernelError::Message("refusing to delete the workspace root".into()));
        }
    }
    ...
    if meta.is_dir() && !meta.file_type().is_symlink() {
        if recursive {
            ...
            std::fs::remove_dir(path)
                .map_err(|e| KernelError::Message(format!("`{}` is a directory: {e}", path.display())))
        }
    } else {
...
mod tests {
    ...
    #[test]
    fn base64_roundtrip() {
        for input in [&b""[..], b"f", b"fo", b"foo", b"foob", b"fooba", b"foobar", &[0u8, 255, 10, 7]] {
